...
export default function CartProduct({ _id, name, images, quantity, price }) {
    ...
    const updateQuantityVal = (quantity) => {
        fetch('http://localhost:8008/api/v2/product/cartproduct/quantity', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                email: 'harshitabhingare21@gmail.com',
                productId: _id,
                quantity,
            }),
        })
	...
	return (
		<div className="h-max w-full p-4 flex justify-between border-b border-neutral-300 bg-neutral-100 rounded-lg">
			<div className="flex flex-col gap-y-2">
				<img
					src={`http://localhost:8008${currentImage}`} // Ensure the URL is correct\
					alt={name}
