...
const OrderConfirmation = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const addressResponse = await axios.get('http://localhost:8008/api/v2/user/addresses', {
                    params: { email: email },
                });
                ...
                const cartResponse = await axios.get('http://localhost:8008/api/v2/product/cartproducts', {
                    params: { email: email },
                });
                ...
                const processedCartItems = cartData.cart.map(item => ({
                    ...
                    images: item.productId.images.map(imagePath => `http://localhost:8008${imagePath}`),
    ...
    const handlePlaceOrder = async () => {
        try {
            ...
            const response = await axios.post('http://localhost:8008/api/v2/orders/place-order', payload);
