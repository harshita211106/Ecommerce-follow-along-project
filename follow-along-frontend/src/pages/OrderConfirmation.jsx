        ...
        const fetchData = async () => {
            try {
                ...
                const addressResponse = await axios.get(
                    'http://localhost:8008/api/v2/user/addresses',
                    {
                        params: { email: email },
                    }
                );
                ...
                const cartResponse = await axios.get(
                    'http://localhost:8008/api/v2/product/cartproducts',
                    {
                        params: { email: email },
                    }
                );
                ...
                const processedCartItems = cartData.cart.map((item) => ({
                    ...
                    images: item.productId.images.map(
                        (imagePath) => `http://localhost:8008${imagePath}`
                    ),
    ...
    const handlePlaceOrder = async (paymentType = 'cod', paypalOrderData = null) => {
        try {
            ...
            const response = await axios.post(
                'http://localhost:8008/api/v2/orders/place-order',
                payload
            );
                        ...
                        {paymentMethod === 'paypal' && (
                            <div className='mt-4' style={{ maxWidth: '500px' }}>
                                <PayPalScriptProvider
                                    options={{
                                        // own client id
                                        'client-id': 'AUZonN-bIqq0w3M99VD1ZTJ-_GsUGoWD4gJWsBAqCO0oSOw_dteJ7PLVkDwqaGWpqF1WHcpnB7qlLeGL',
                                    }}
                                    >
