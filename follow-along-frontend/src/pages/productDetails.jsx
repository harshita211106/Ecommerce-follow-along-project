...
export default function ProductDetails() {
	...
	const email = "harshitabhingare21@gmail.com";   

	useEffect(() => {
		const fetchProduct = async () => {
			try {
				const response = await axios.get(
					`http://localhost:8008/api/v2/product/product/${id}`
				);
	...
	const addtocart = async () => {
		try {
			const response = await axios.post(
				"http://localhost:8008/api/v2/product/cart",
				{
					userId: email,
					productId: id,
					quantity: quantity,
					...
					<div className="md:flex select-none">
						...
						<div className="w-full bsm:w-2/3 md:w-1/3 rounded-lg">
							{product.images && product.images.length > 0 ? (
								<img
									src={`http://localhost:8008${product.images[0]}`}
									alt={product.name}
