...
const CreateProduct = () => {
    ...
    useEffect(() => {
        if (isEdit) {
            axios
                .get(`http://localhost:8008/api/v2/product/${id}`)
                .then((response) => {
                    ...
                    if (p.images && p.images.length > 0) {
                        setPreviewImages(
                            p.images.map((imgPath) => `http://localhost:8008${imgPath}`)
                        );
                    }
                })
                .catch((err) => {
        ...
        try {
            if (isEdit) {
                const response = await axios.put(
                    `http://localhost:8008/api/v2/product/update-product/${id}`,
                    formData,
                    {
                ...
                const response = await axios.post(
                    "http://localhost:8008/api/v2/product/create-product",
                    formData,
                    {
                        headers: { "Content-Type": "multipart/form-data" },
                    }
